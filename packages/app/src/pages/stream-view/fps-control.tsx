...
interface FpsControllProps {
  deviceId: string;
}

export function FpsControll({ deviceId }: FpsControllProps) {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState("");
  const { isMobileLandscape } = useMobileLandscape();
  const { data: device } = useDevice(deviceId);
  const { fps, setFps, refreshFps, isLoading } = useFps(deviceId);
  ...
  const showCreateOption = useMemo(() => {
    ...
    const isNotPreset = !device?.information?.fps_presets?.[search];
    return isNotPreset;
  }, [device?.information?.fps_presets, search]);

  const handleRefresh = async () => {
    await refreshFps();
  };

  const fpsPresets = useMemo(() => {
    if (!device?.information?.fps_presets) return null;

    return Object.entries(device.information.fps_presets).map(
      ([label, value]) => ({
        value: value.toString(),
        label,
      })
    );
  }, [device?.information?.fps_presets]);

  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-2">
        <Label className="text-xs sm:text-sm">FPS</Label>
        <Button
          variant="ghost"
          size="icon"
        ...
        >
          ...
          />
          <span className="sr-only">Refresh fps</span>
        </Button>
      </div>
