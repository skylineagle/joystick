...
export function optionFilterFn<TData>(
  ...
  if (typeof value === "boolean") {
    return __optionFilterFn(value.toString(), filterValue);
  }

  if (isColumnOption(value)) {
    return __optionFilterFn(
      typeof value.value === "boolean" ? value.value.toString() : value.value,
      filterValue
    );
  }
  ...
  return __optionFilterFn(
    typeof sanitizedValue.value === "boolean"
